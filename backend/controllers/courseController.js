...
export const createLecture = async (req, res) => {
  ...
  try {
    ...
    //------------------------------------------------
    // ⭐ CREATE WITH COURSE REFERENCE
    //------------------------------------------------

    const lecture = await Lecture.create({

      lectureTitle,
      course: courseId   // ⭐⭐⭐ THIS FIXES YOUR ERROR

    });

    //------------------------------------------------
    // Add lecture inside course
    //------------------------------------------------
...
//After this code new changes


//For enrollment purpose made this controller 

// ENROLL COURSE (NO PAYMENT)
export const enrollCourse = async (req, res) => {
  try {
    const { courseId } = req.params;   // course user wants to enroll
    const userId = req.userId;         // logged-in user (from isAuth)
    ...
    console.log(error);
    return res.status(500).json({ message: "Enroll course error" });
  }
};
